...
"""
DCGAN Tutorial
Source: https://github.com/pytorch/tutorials/edit/master/beginner_source/dcgan_faces_tutorial.py
==============
...
# Alternative Generator with upsampling instead of transpose convolution
class Generator2(nn.Module):
    def __init__(self, ngpu, input_dim, out_channels):
        super(Generator2, self).__init__()
        self.ngpu = ngpu
        self.lin1 = nn.Linear(input_dim, 256 * 8 * 8)
...
if __name__ == '__main__':
    ...
    dataroot = "/home/marcel/Uni/Master/3.Semester/DGM/Emoji-Gen/Emoji-Generator/emoji_data/emojis_jpg_root"
    ...
    netG = Generator2(ngpu, 100, 3).to(device)
    ...
    for epoch in range(num_epochs):
        ...
        for i, data in enumerate(dataloader, 0):
            ...
            if (iters % 500 == 0) or ((epoch == num_epochs - 1) and (i == len(dataloader) - 1)):
                ...
                torch.save(netD.state_dict(), dis_weights_path)
...
def inference(x_in, output_path, nz=100, weights=None, map_location=None):
    """
    Generate images from a batch of random noise
    :param x_in: batch of random noise of size (batch_size x 1 x noise_dim)
    :param output_path: path where the result is stored
    """
    netG = Generator(0)
    if weights:
        netG.load_state_dict(torch.load(weights, map_location=map_location))
    if x_in is None:
        x_in = torch.randn(1, nz, 1, 1)
    result = netG(x_in)
    save_image_batch(result, output_path=output_path)
